...
The selected-measurement form also keeps its focused, unsubmitted field values across an asynchronous render for the same stable measurement ID. This prevents late electrical-overlay responses from replacing in-progress text without creating another stored draft. Overlay readiness is exposed as `aria-busy`; changing measurement or leaving Measurements does not carry the form buffer into another context.
