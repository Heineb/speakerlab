...
UI Workflow Consolidation v1 changes presentation, not the versioned Signal Flow storage or server contracts. The `signal-flow` extension is labelled **Speaker Design** and exposes local Design, Measurements and Review views over the same `routing-ui-state` draft. Workspace selection and Advanced disclosure state remain transient browser presentation state. Stop and Validate v1 retains only the selected output and its active design section in tab-scoped session storage, validates restored identifiers against the current draft and stores no design or assistant data there.

The selected-measurement form keeps edited, unsubmitted field values across an asynchronous render for the same stable measurement ID, whether or not a field still owns focus. Values are buffered only when they differ from the authoritative draft. This prevents late electrical-overlay responses from replacing in-progress metadata without creating another stored draft. Overlay readiness is exposed as `aria-busy`; changing measurement or leaving Measurements does not carry the form buffer into another context.
