...
Assisted Crossover stays inside the existing Crossover section and adds no navigation or dashboard. The primary flow names two measurements, the usable candidate region, at most three distinct alternatives, current-versus-suggested prediction, confidence, cautions and explicit Apply/Close actions. Magnitude-only and phase-aware predictions must be named differently; magnitude-only may not imply complex acoustic summation, polarity or delay. Numerical scores and timing diagnostics start under collapsed `Advanced`. Acceptance creates ordinary unsaved crossover and explicitly shown processing values; preview and rejection change nothing.

## Consolidated Speaker Design workflow

The inherited global shell remains stable. Loudspeaker work is presented as **Speaker Design** with three local, outcome-oriented views: **Design**, **Measurements** and **Review**. They share one ordinary draft and are not separate application modules or configuration stores.

Design shows a compact output selector and exactly one selected output editor. Its single-open sequence is **Output & routing**, **Crossover**, **Level & timing**, **Parametric EQ** and **Driver Protection**. Collapsed summaries carry useful state; contextual assistance remains inside the section it supports. Measurements keeps import, inspect and assign primary, with merge and routes to assistance as contextual actions. Review is derived, concise and links back to editing context.

Use one standard **Advanced** disclosure, collapsed by default with semantic expanded state. Algorithm settings, raw metrics, provenance, compiler/mapping and transport/readback detail belong there. Validation errors, stale state, physical-deployment blockers and material safety warnings remain outside it. Opening or closing Advanced never mutates the design.

Canonical language is **Output** for the signal/hardware path, **Driver** for acoustic role, **Level & timing** for Level/Delay/Polarity, **Measurement** for imported observation and **derived response** for a merge. **Measured**, **Predicted** and **Simulated** identify different evidence. Design is **Saved**, **Unsaved** or **Error**; deployment is **Simulated** or **Blocked** unless a physical statement is explicitly qualified.
